...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (userSelectionContainer) { // We are on index.html
        ...
        if (userNumber && parseInt(userNumber) > 0 && parseInt(userNumber) <= TOTAL_USERS) {
            ...
            document.getElementById('user-page-title').textContent = `Votación Eurovisión - ${["Santi", "Rocío", "Belén", "Ricardo"][userNumber - 1]}`;
...
function renderUserNavigationLinks(container) {
    for (let i = 1; i <= TOTAL_USERS; i++) {
        ...
        link.textContent = `${["Santi", "Rocío", "Belén", "Ricardo"][i - 1]}`;
...
function renderSingleUserPanel(container, userId, userNumber) {
    ...
    saveButton.textContent = `Guardar Votos de ${["Santi", "Rocío", "Belén", "Ricardo"][userNumber - 1]}`;
